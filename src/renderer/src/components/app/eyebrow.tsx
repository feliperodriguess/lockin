...
function AfterLine({ width = 28, className }: AfterLineProps): React.JSX.Element {
	return (
		<span
			className={cn("inline-block shrink-0 bg-[var(--color-accent)] align-middle", className)}
...
function Eyebrow({ children, line, className }: EyebrowProps): React.JSX.Element {
	...
	return (
		<div className={cn("flex items-center gap-2", className)}>
			<span className="font-mono text-[10px] font-semibold uppercase leading-none tracking-[0.16em] text-[var(--fg-3)] whitespace-nowrap">
				{children}
			</span>
